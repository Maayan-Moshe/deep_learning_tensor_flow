...
def prepare_hmap_data(fpath):
    
    orig_data = np.load(fpath).tolist()
    hmaps = list()
    true_z = list()
    for val in orig_data.itervalues():
        append_hmap(val, hmaps, true_z)
    return {'hmaps': np.array(hmaps), 'stl_z': np.array(true_z)}
    
def append_hmap(val, hmaps, true_z):
    '''
    It is assumed but not enforced that the ordering is the same as in AX_INDEXES.
    '''
    if 'stl_z' not in val:
        return
    hmaps.append(val['height_mat_mm'])
