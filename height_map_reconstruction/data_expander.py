...
def expand_data(reduced_average_data, pre_reduced_dat, pre_shape):
    with tf.name_scope('expanding_data'):
        ...
        expanded_data = {key: expander.get_expand_hmap(value, kernel_size = 3) \
                for key, value in reduced_average_data.items() if 'hmap' in key}
        expanded_data.update({'pre_' + key: expander.get_expand_hmap(value, kernel_size = 3) \
                for key, value in pre_reduced_dat.items() if 'hmap' in key})
