...
def stack_input(previous_data):
    with tf.name_scope('stacking_input'):
        prev_tensors = [reshape_hmap(value) for value in previous_data.values()]
        stacked_input = tf.concat(prev_tensors, axis = 3, name = 'stacked_input')
                    
        return stacked_input
    
def reshape_hmap(pre_hmap):
    with tf.name_scope('reshaping_hmap'):
        pre_shape = pre_hmap.get_shape()
        if len(pre_shape) == 4:
            return pre_hmap
        ps = [-1, pre_shape[1].value, pre_shape[2].value, 1]
        hmap = tf.reshape(pre_hmap, ps)
        return hmap
